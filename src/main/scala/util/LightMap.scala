...
object LightMap {
    ...
    def rasterize(shadow: (Ray, Array[Point], Ray))(lightMap: Array[Array[(Double, Boolean)]], lightCount: Int)(width: Int, height: Int): Array[Array[(Double, Boolean)]] = {
        ...
        if (getBorderIntersection(shadow._1)(width, height).x > 0 && getBorderIntersection(shadow._3)(width, height).x > 0 && shadow._1.angleTo(shadow._3) > math.Pi / 2d) activeEdgeList ::= getActiveEdge(new Ray(new Point(0, 0), new Point(0, height)))
